# AGENTS.md — Seeing Through Deepfakes
...
- A Codex skill `deepfake-detection-framework` (outside this repo) holds the report's exact fusion formulas and report writing style — load it when touching fusion math or writing thesis prose.
...
- **The canonical fused decision threshold is 0.16** (David, 2026-09-06; was 0.55 up to 2026-09-06), set in `src/genai_detection/integration_pipeline/config.py` and the notebook config cells. Lowered from 0.55 after a fine 0.10→0.20 sweep on both the in-distribution combined_dataset/test cohort (n=200) and an external CommunityForensics-Eval cohort (n=200 across 20 generators) identified 0.16 as the dual-cohort optimum on all three aggregate criteria (avg Youden's J, avg F1, avg accuracy) — see `outputs/visual_classifier_threshold_sweep/summary_results.json`. The historical 0.55 numbers are retained verbatim in `src/genai_detection/visual_module/outputs/*.json` and in `outputs/robustness_visual_classifier_thr055_historical/`. The report's printed 0.5 predates the 2026-07 decision; historic 0.25/0.45 values were testing artefacts from the mispaired-model era. Don't change it without David.
