...
- Stage 2 (as deployed): `P(AI) ≥ 0.16 → D₂` (AI-generated) else `D₁` (not AI). Only `D₂` triggers Module 4. The report's §3.4 prints 0.5 as period terminology; the deployed threshold has been 0.55 (2026-07-19 → 2026-09-06) and is now 0.16 (see `config.py` docstring for the sweep evidence).
...
- **The canonical fused decision threshold is 0.16** (David, 2026-09-06; was 0.55 canonical from 2026-07-19; centralised in `src/genai_detection/integration_pipeline/config.py`). The change followed a fine 0.10→0.20 sweep on two cohorts (in-distribution combined_dataset/test n=200; external CommunityForensics-Eval n=200 across 20 generators) — 0.16 optimises the mean Youden's J, F1 and accuracy across both. Historical 0.55 numbers are retained verbatim under `src/genai_detection/visual_module/outputs/` and `outputs/robustness_visual_classifier_thr055_historical/`. The report's printed 0.5 predates the 2026-07 decision; the old 0.25 (app) / 0.45 (bulk notebook) were testing values from the mispaired-model era.
